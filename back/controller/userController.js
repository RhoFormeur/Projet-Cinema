...
exports.pageProfil = async function (req, res) {
    ...
    if (req.session.user.is_admin===1) res.render("pages/profil", { films, series, animes, user, layout: 'layout_admin' })
    else res.render("pages/profil", { films, series, animes, user, layout: 'layout_user' })
}
...
exports.deleteProfil = async function (req, res) {
    ...
    else res.render('/', { flash: "Votre compte a bien été supprimé !" })
}
...
exports.updateLike = async function (req, res) {
    const { id } = req.params
    const { is_liked } = req.body
    await db.query(`UPDATE likes SET is_liked = ${is_liked} WHERE id_article=${id} AND id_user=${req.session.user.id};`)
    res.render("pages/fiche_article", { data: await RenderArticle(id) })
}